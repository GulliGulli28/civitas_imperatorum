...
class ScenarioBatimentenFeu:
    ...
    Bâtiment = House(0,0,1)
    Bâtiment.fire_risk =0.3
    map_build.add_build(Bâtiment)
    for i in range(0,40):
        road = Road(1,i)
        map_build.add_build(road)
    print("batiment placé en:", Bâtiment.positionX, Bâtiment.positionY)
    ...
    while k:
        ...
        Bâtiment.risque_feu()
        Bâtiment.update_risk()
        print("Le risque de feu de Bâtiment est maintenant de: ", Bâtiment.fire_risk)
        print("Le batiment est en feu: ", Bâtiment.is_on_fire)
