...
class mapBuilding:
    ...
    def add_build(self, new_build):
        print("non")
        # if not isinstance(new_build, Road): #TODO a decommenter lorsque l'affichage sera correct cela permet de
        #  TODO vérifier qu'il y a une route a cote du batiment mais trop chiant pour les test
        #  road_near = False for i in
        #  range(new_build.size):  # check that there is a road next to the building for y in range(new_build.size):
        #  if len(self.get_direction(new_build.positionX + i, new_build.positionY + y)) > 0: road_near = True if len(
        #  self.get_direction(new_build.positionX + y, new_build.positionY + i)) > 0: road_near = True if not
        #  road_near: return 0
